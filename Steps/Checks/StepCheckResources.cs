    ...
    public class StepCheckResources : SMissionStep
    {
        ...
        public override void FirstSetup()
        {
            try
            {
                ...
                //Debug_SMissions.Log(KickStart.ModID + ": StepCheckResources - ChunkType assigned is " + ((ChunkTypes)SMission.InputNum).ToString());
                //Debug_SMissions.Log(KickStart.ModID + ": StepCheckResources - current count is " + Singleton.Manager<ManStats>.inst.GetNumResourcesHarvested((ChunkTypes)SMission.InputNum) + ".");
            }
            catch (Exception e)
            {   // not assigned correctly
                SMUtil.Assert(true, SMission.LogName, KickStart.ModID + ": Error in input (ChunkTypes) InputNum in mission " + 
        ...
        {   // check player mined chunks
            try
            {
                Mission.VarIntsActive[SMission.SetMissionVarIndex1] = Singleton.Manager<ManStats>.inst.GetNumResourcesHarvested((ChunkTypes)SMission.InputNum) - SMission.SavedInt;
                if (Mission.VarIntsActive[SMission.SetMissionVarIndex2] <= Mission.VarIntsActive[SMission.SetMissionVarIndex1])
            ...
            {   // it's being triggered wrong - testing
                ...
                try
                {
                    Mission.VarIntsActive[SMission.SetMissionVarIndex1] = SMission.SavedInt;
                    if (Mission.VarIntsActive[SMission.SetMissionVarIndex2] <= Mission.VarIntsActive[SMission.SetMissionVarIndex1])
                        SMUtil.ConcludeGlobal3(ref SMission);
                }
                catch (IndexOutOfRangeException e)
                {
                    SMUtil.Assert(true, SMission.LogName, KickStart.ModID + ": Error in output [SetMissionVarIndex1] or [SetMissionVarIndex2] in mission " + Mission.Name +
