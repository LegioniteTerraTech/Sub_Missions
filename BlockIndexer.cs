...
namespace TerraTechETCUtil
{
    ...
    public class BlockIndexer : MonoBehaviour
    {
        ...
        private static bool isBlockInjectorPresent => Sub_Missions.KickStart.isBlockInjectorPresent;
        private static char FolderDivider => Sub_Missions.SMissionJSONLoader.up;
        private static bool Compiled = false;

        /// <summary>
        /// Searches the ENTIRE GAME for the block's prefab based on root GameObject name.
        /// </summary>
        ...
        public static TankBlock StringToBlockPrefab(string mem)
        {
            if (StringToBlockType(mem, out BlockTypes BT))
            {
                return ManSpawn.inst.GetBlockPrefab(BT);
            }
            return null;
        }
